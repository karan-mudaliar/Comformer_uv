...
# Output/error logs
OUT_FILE="eComformer_WF_space_group_no_zsym_${JOB_ID}.txt"
ERR_FILE="eComformer_WF_space_group_no_zsym_err_${JOB_ID}.txt"

# Start logging
{
echo "===== eComformer WF Space Group No ZSym Training Started: $(date) ====="
...
echo "🚀 Running train_eComformer_WF_space_group_no_zsym.py ..."
nohup python -u comformer/scripts/training_split_scripts/train_eComformer_WF_space_group_no_zsym.py &

echo "===== eComformer WF Space Group No ZSym Training Finished: $(date) ====="
} > "$OUT_FILE" 2> "$ERR_FILE"
