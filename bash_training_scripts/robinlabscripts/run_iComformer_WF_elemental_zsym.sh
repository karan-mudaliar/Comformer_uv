...
# Output/error logs
OUT_FILE="iComformer_WF_elemental_zsym_${JOB_ID}.txt"
ERR_FILE="iComformer_WF_elemental_zsym_err_${JOB_ID}.txt"

# Start logging
{
echo "===== iComformer WF Elemental ZSym Training Started: $(date) ====="
...
echo "🚀 Running train_iComformer_WF_elemental_zsym.py ..."
nohup python -u comformer/scripts/training_split_scripts/train_iComformer_WF_elemental_zsym.py &

echo "===== iComformer WF Elemental ZSym Training Finished: $(date) ====="
} > "$OUT_FILE" 2> "$ERR_FILE"
