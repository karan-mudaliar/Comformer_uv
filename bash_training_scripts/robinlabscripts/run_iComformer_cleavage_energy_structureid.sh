...
# Output/error logs
OUT_FILE="iComformer_cleavage_energy_structureid_${JOB_ID}.txt"
ERR_FILE="iComformer_cleavage_energy_structureid_err_${JOB_ID}.txt"

# Start logging
{
echo "===== iComformer Cleavage Energy Structure ID Training Started: $(date) ====="
...
echo "🚀 Running train_iComformer_cleavage_energy_structureid.py ..."
nohup python -u comformer/scripts/training_split_scripts/train_iComformer_cleavage_energy_structureid.py &

echo "===== iComformer Cleavage Energy Structure ID Training Finished: $(date) ====="
} > "$OUT_FILE" 2> "$ERR_FILE"
