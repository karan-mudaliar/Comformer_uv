...
# Output/error logs
OUT_FILE="eComformer_cleavage_energy_pt_group_${JOB_ID}.txt"
ERR_FILE="eComformer_cleavage_energy_pt_group_err_${JOB_ID}.txt"

# Start logging
{
echo "===== eComformer Cleavage Energy PT Group Training Started: $(date) ====="
...
echo "🚀 Running train_eComformer_cleavage_energy_pt_group.py ..."
nohup python -u comformer/scripts/training_split_scripts/train_eComformer_cleavage_energy_pt_group.py &

echo "===== eComformer Cleavage Energy PT Group Training Finished: $(date) ====="
} > "$OUT_FILE" 2> "$ERR_FILE"
