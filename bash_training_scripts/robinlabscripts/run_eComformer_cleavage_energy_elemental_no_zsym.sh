...
# Output/error logs
OUT_FILE="eComformer_cleavage_energy_elemental_no_zsym_${JOB_ID}.txt"
ERR_FILE="eComformer_cleavage_energy_elemental_no_zsym_err_${JOB_ID}.txt"

# Start logging
{
echo "===== eComformer Cleavage Energy Elemental No ZSym Training Started: $(date) ====="
...
echo "🚀 Running train_eComformer_cleavage_energy_elemental_no_zsym.py ..."
nohup python -u comformer/scripts/training_split_scripts/train_eComformer_cleavage_energy_elemental_no_zsym.py &

echo "===== eComformer Cleavage Energy Elemental No ZSym Training Finished: $(date) ====="
} > "$OUT_FILE" 2> "$ERR_FILE"
