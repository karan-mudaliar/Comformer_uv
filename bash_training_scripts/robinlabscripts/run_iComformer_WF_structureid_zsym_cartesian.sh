...
# Output/error logs
OUT_FILE="iComformer_WF_structureid_zsym_cartesian_${JOB_ID}.txt"
ERR_FILE="iComformer_WF_structureid_zsym_cartesian_err_${JOB_ID}.txt"

# Start logging
{
echo "===== iComformer WF Structure ID ZSym Cartesian Training Started: $(date) ====="
...
echo "🚀 Running train_iComformer_WF_structureid_zsym_cartesian.py ..."
echo "🆕 Using NEW cartesian z-symmetry breaking method"
nohup python -u comformer/scripts/training_split_scripts/train_iComformer_WF_structureid_zsym_cartesian.py &

echo "===== iComformer WF Structure ID ZSym Cartesian Training Finished: $(date) ====="
} > "$OUT_FILE" 2> "$ERR_FILE"
