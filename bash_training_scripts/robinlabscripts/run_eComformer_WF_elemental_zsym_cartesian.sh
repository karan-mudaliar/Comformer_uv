...
# Output/error logs
OUT_FILE="eComformer_WF_elemental_zsym_cartesian_${JOB_ID}.txt"
ERR_FILE="eComformer_WF_elemental_zsym_cartesian_err_${JOB_ID}.txt"

# Start logging
{
echo "===== eComformer WF Elemental ZSym Cartesian Training Started: $(date) ====="
...
echo "🚀 Running train_eComformer_WF_elemental_zsym_cartesian.py ..."
echo "🆕 Using NEW cartesian z-symmetry breaking method"
nohup python -u comformer/scripts/training_split_scripts/train_eComformer_WF_elemental_zsym_cartesian.py &

echo "===== eComformer WF Elemental ZSym Cartesian Training Finished: $(date) ====="
} > "$OUT_FILE" 2> "$ERR_FILE"
